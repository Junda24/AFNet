...
class DDAD(data.Dataset):
    def __init__(self, opt, is_train):
        ...
        self.opt = opt
        self.json_path = "data_split/DDAD_video.json"
        self.data_path_root = self.opt.data_path
        ...
        self.file_names = [x for x in self.file_names if 'timestamp' in x.keys() and 'timestamp_back' in x.keys() and 'timestamp_forward' in x.keys()]
    ...
    def get_k_ori_centercrop(self, k_raw, inputs):
